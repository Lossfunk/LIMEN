"""MAP-Elites database with island model for LIMEN.
...
from limen.config import DatabaseConfig
from limen.evaluator import CandidateResult, EvalStage
...
class Program:
    ...
    def get_fitness(self, key: str = "success_rate") -> float:
        """Get fitness using a configurable metric key."""
...
class ProgramDatabase:
    ...
    def __init__(self, config: DatabaseConfig, fitness_key: str = "success_rate"):
        self.config = config
    ...
    def _get_fitness(self, metrics_or_program) -> float:
        """Get fitness value using the configured fitness_key.

        Accepts either a Program or a metrics dict.
        """
        if isinstance(metrics_or_program, Program):
            return metrics_or_program.get_fitness(self.fitness_key)
        if isinstance(metrics_or_program, dict):
            return metrics_or_program.get(self.fitness_key, 0.0)
        return 0.0
    ...
    def add(
        ...
        if self._get_fitness(result.metrics or {}) == 0.0:
            self.add_failure(
                FailedProgram(
                    code=result.code,
                    metrics=result.metrics,
        ...
        if not should_place:
            ...
            if existing is None or program.get_fitness(self.fitness_key) > existing.get_fitness(self.fitness_key):
                should_place = True
    ...
    def _sample_weighted(self) -> Optional[Program]:
        ...
        fitnesses = [self._get_fitness(p) for p in programs]
    ...
    def get_top_programs(
        ...
        candidates.sort(key=lambda p: self._get_fitness(p), reverse=True)
    ...
    def get_diverse_programs(self, n: int = 2) -> List[Dict[str, Any]]:
        ...
        for island_idx, island_grid in enumerate(self.grid):
            for key, pid in island_grid.items():
                if pid in self.programs:
                    p = self.programs[pid]
                    if key not in cell_programs or self._get_fitness(p) > self._get_fitness(cell_programs[key]):
                        cell_programs[key] = p
    ...
    def migrate(self) -> int:
        ...
        for src_idx in range(num_islands):
            ...
            src_programs = sorted(
                ...
                key=lambda p: self._get_fitness(p),
                reverse=True,
            )
            ...
            for prog in migrants:
                ...
                if not should_place:
                    existing = self.programs.get(existing_id)
                    if existing is None or self._get_fitness(migrant) > self._get_fitness(existing):
                        should_place = True
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "best_fitness": self._get_fitness(best) if best else None,
        ...
        if len(self.archive) < self.config.archive_size:
            ...
            for pid in self.archive:
                ...
                if p is None:
                    worst_id = pid
                    break
                if self._get_fitness(p) < worst_fitness:
                    worst_fitness = self._get_fitness(p)
                    worst_id = pid

            if worst_id and (
                self.programs.get(worst_id) is None
                or self._get_fitness(program) > worst_fitness
            ):
    ...
    def _update_best(self, program: Program) -> None:
        ...
        if best is None or self._get_fitness(program) > self._get_fitness(best):
            self.best_program_id = program.id
