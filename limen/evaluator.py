...
from limen.crash_filter import CrashFilterResult, run_crash_filter
...
class CascadeEvaluator:
    ...
    def __init__(self, config: Config, adapter: EnvAdapter):
        ...
        self.fitness_key = adapter.fitness_key()
        self.fitness_type = adapter.fitness_type()
    ...
    def _cascade_evaluate(
        ...
        short_fitness = short_metrics.get(self.fitness_key, 0.0)
        if self.fitness_type == "reward":
            logger.info(
                "Short training done: fitness=%.1f, length=%.1f, time=%.1fs",
                short_fitness,
        ...
        # Threshold check
        if short_fitness < threshold:
            if self.fitness_type == "reward":
                logger.info(
                    "Short training fitness %.1f < threshold %.1f — rejected",
                    short_fitness,
                    threshold,
                )
            else:
        ...
        if self.fitness_type == "reward":
            logger.info(
                "Passed threshold (%.1f >= %.1f). Running full training (%s steps, %d seed(s))...",
                short_fitness,
                threshold,
        ...
        full_fitness = full_metrics.get(self.fitness_key, 0.0)
        if self.fitness_type == "reward":
            logger.info(
                "Full training done: fitness=%.1f, length=%.1f, time=%.1fs",
                full_fitness,
    ...
    def _full_evaluate(
        ...
        eval_fitness = metrics.get(self.fitness_key, 0.0)
        if self.fitness_type == "reward":
            logger.info(
                "Full training done: fitness=%.1f, length=%.1f, time=%.1fs",
                eval_fitness,
