"""Evolution controller — the main loop for LIMEN.

Ties together: LLM generation → crash filter → cascade training → MAP-Elites database.

Usage:
    config = Config.from_yaml("configs/easy_pickup.yaml")
...
from limen.adapters import get_adapter
...
from limen.evaluator import CandidateResult, CascadeEvaluator, EvalStage
from limen.evolution_trace import EvolutionTracer
from limen.llm_client import LLMEnsemble, LLMResponse
...
class EvolutionController:
    ...
    def __init__(
        ...
        self.fitness_type = self.adapter.fitness_type()
        self.fitness_key = self.adapter.fitness_key()
        self.db = ProgramDatabase(config.database, fitness_key=self.fitness_key)
    ...
    def _record_tokens(self, response) -> None:
        """Accumulate token usage globally and per-model."""
        self.total_llm_tokens += response.input_tokens + response.output_tokens
        self.total_input_tokens += response.input_tokens
        self.total_output_tokens += response.output_tokens
        bucket = self.tokens_by_model.setdefault(response.model, [0, 0])
        bucket[0] += response.input_tokens
        bucket[1] += response.output_tokens
    ...
    def _run_default_baseline(self) -> None:
        ...
        baseline_fitness = metrics.get(self.fitness_key, 0.0)
        if self.fitness_type == "reward":
            logger.info(
                "Default baseline complete: fitness=%.1f, length=%.1f, time=%.1fs",
                baseline_fitness,
        ...
        if (
            ...
            if retry_response.code is not None:
                ...
                if retry_result.stage != EvalStage.CRASHED:
                    result = retry_result
                    retry_fitness = retry_result.metrics.get(self.fitness_key, 0.0) if retry_result.metrics else 0.0
                    logger.info(
                        "Retry succeeded: %s fitness=%.4f",
    ...
    def _log_iteration(
        ...
        best = self.db.get_best_program()
        best_fitness = self.db._get_fitness(best) if best else 0.0
        result_fitness = result.metrics.get(self.fitness_key, 0.0) if result.metrics else 0.0
        ...
        if result.stage == EvalStage.CRASHED:
            ...
            if self.fitness_type == "reward":
                logger.info(
                    "[Iter %d] REJECTED (fitness=%.1f) | best=%.1f | %.1fs",
                    self.iteration,
                    result_fitness,
                    best_fitness,
                    iter_time,
            ...
            if self.fitness_type == "reward":
                logger.info(
                    "[Iter %d] %s fitness=%.1f length=%.1f | best=%.1f | %.1fs%s",
                    self.iteration,
                    result.stage.value,
                    result_fitness,
    ...
    def _log_summary(self) -> None:
        ...
        from limen.pricing import estimate_cost
        est_cost, unknown = estimate_cost(
            {m: tuple(tok) for m, tok in self.tokens_by_model.items()},
            self.config.llm.api_base,
        )
        if unknown:
            logger.info("  Est. LLM cost: $%.4f (unknown price for: %s)", est_cost, ", ".join(unknown))
        else:
            logger.info("  Est. LLM cost: $%.4f", est_cost)
        ...
        if best:
            if self.fitness_type == "reward":
                logger.info("  Best fitness: %.1f", self.db._get_fitness(best))
            else:
                logger.info("  Best success rate: %.0f%%", self.db._get_fitness(best) * 100)
    ...
    def _save_best_interface(self) -> None:
        ...
        fitness_val = self.db._get_fitness(best)
        if self.fitness_type == "reward":
            fitness_label = f"Fitness:      {fitness_val:.1f}"
        else:
            fitness_label = f"Success rate: {fitness_val:.0%}"
        header = (
            f'"""\n'
            f'Best MDP Interface — evolved by LIMEN\n'
            f'\n'
    ...
    def load_checkpoint(self, checkpoint_dir: str) -> None:
        ...
        if meta_path.exists():
            ...
            self.tokens_by_model = {
                m: list(tok) for m, tok in meta.get("tokens_by_model", {}).items()
            }
